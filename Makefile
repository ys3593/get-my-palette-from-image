...
lint:  ## run static analysis with flake8
	python -m black --check my_palette setup.py
	python -m flake8 my_palette setup.py

# Alias
lints: lint

format:  ## run autoformatting with black
	python -m black my_palette/ setup.py
...
annotate:  ## run type checking
	python -m mypy ./my_palette

#########
# TESTS #
#########
test: ## clean and run unit tests
	python -m pytest -v my_palette/tests

coverage:  ## clean and run unit tests with coverage
	python -m pytest -v my_palette/tests --cov=my_palette --cov-branch --cov-fail-under=75 --cov-report term-missing
